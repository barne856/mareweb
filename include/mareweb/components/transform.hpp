...
concept transformable = requires(T t) {
  { t.get_position() } -> std::same_as<vec3_t<units::length>>;
  { t.get_scale() } -> std::same_as<vec3_t<units::length>>;
  { t.get_translation_matrix() } -> std::same_as<mat4>;
  { t.get_rotation_matrix() } -> std::same_as<mat4>;
  { t.get_scale_matrix() } -> std::same_as<mat4>;
  { t.get_transformation_matrix() } -> std::same_as<const mat4 &>;
  { t.get_normal_matrix() } -> std::same_as<mat3>;
  { t.get_view_matrix() } -> std::same_as<mat4>;
  {
    t.face_towards(std::declval<vec3_t<units::length>>(), std::declval<vec3_t<units::length>>())
  } -> std::same_as<void>;
  { t.translate(std::declval<vec3_t<units::length>>()) } -> std::same_as<void>;
  { t.set_position(std::declval<vec3_t<units::length>>()) } -> std::same_as<void>;
...
public:
  ...
  [[nodiscard]] auto get_translation_matrix() const -> mat4;
  [[nodiscard]] auto get_rotation_matrix() const -> mat4;
  [[nodiscard]] auto get_scale_matrix() const -> mat4;
  [[nodiscard]] auto get_transformation_matrix() const -> const mat4 &;
  void set_transformation_matrix(const mat4 &transformation_matrix);
  ...
  void set_unit_length(const units::length &unit_length);

private:
  mat4 m_transformation_matrix;
  units::length m_unit_length;
};
