...
public:
  ...
  void add_uniform_buffer(uint32_t binding_index, std::shared_ptr<uniform_buffer> buffer);
  void update_uniform_buffer(uint32_t binding_index, const void *data, size_t size);
...
private:
  ...
  std::unordered_map<uint32_t, std::shared_ptr<uniform_buffer>> m_uniform_buffers;
};
